...
cpm <- function(mtx.count,verbose=FALSE){
  ...
  count.sum <- colSums(mtx.count) #apply(mtx.count,2,sum)
  mtx.cpm <- (t(t(mtx.count)/count.sum)*1000000)
  #scale(mtx, center = F, scale = colSums(counts)/1e6L)
...
log2cpm <- function(mtx.cpm,scale=10,verbose=FALSE) {
  ...
  mtx.log2 <- log2((mtx.cpm/scale)+1)
  sketch_mtx(mtx.log2); return(mtx.log2)
}
...
norm_row <- function(mtx,center=TRUE,centerby="mean",scale=FALSE,scaleby=NULL,verbose=TRUE){
  if (verbose) message("running 'norm_row'... return a centered and scaled mtx")
  if(center==TRUE){
    if (centerby=="mean") {
      if (scale==TRUE) {
        if (scaleby=="std") {
...
center_col = function(mtx,by="mean") {
  mtx = as.matrix(mtx)
  if (by == "mean")  by = colMeans(mtx, na.rm = T)
...
center_row = function(mtx, by="mean") {
  #mtx = as.matrix(mtx)
  if (by == "mean")  by = rowMeans(mtx, na.rm = T)
